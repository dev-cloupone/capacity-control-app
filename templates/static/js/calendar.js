...
function weekDayStr(weekDayInt) {
    switch (weekDayInt) {
        case 0:
            return "DOM";
        case 1:
...
const manipulate = () => {
    ...
    for (let i = 1; i <= lastdate; i++) {
        ...
        if(isToday !== "") {
            let weekDay = "";
            if (pageLoad) {
                weekDay = weekDayStr(date.getDay());
            } else {
                const tempSelectDate = new Date(selectedYear, selectedMonth, selectedDay);
                weekDay = weekDayStr(tempSelectDate.getDay());
            }

            const dateMonth = month + 1;
            $("#insertDateInput").val(year + "-" + dateMonth + "-" + i);
            $("#dayViewer-selected-day").text(selectedDay)
            $("#dayViewer-month-title-label").text(months[month] + " " + year);
            $("#dayViewer-selected-weekDay").text(weekDay);
    ...
    $.ajax({
        ...
        success: function (res, status) {
            ...
            $.each(res, function(index, item) {
              ...
              if(index < 4){
                $("#dayViewer-item-" + (index + 1)).text(item.fields.dateStart.split(delimiter).slice(0, 2).join(delimiter))
              }
